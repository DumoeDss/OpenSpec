...
- **CLI-driven next steps** — `rasen status --json` and the apply instructions gain a `nextWorkflows` array, and their human output a trailing `Next: <workflow> — <reason>` hint, filtered to the workflows actually installed in your profile (skipping ahead to the nearest installed step). Localized in en/ja/zh-cn.
...
- **Docs** — the Chinese docs are re-synced to the rasen rebrand (~270 stale `opsx`/`OpenSpec` references across 32 files), the "OPSX" product term is retired (`docs/opsx.md`/`docs/opsx-workflow-guide.md` → `artifact-workflow.md`/`artifact-workflow-guide.md`), and the command-syntax tables show the correct `/rasen-propose` form for Claude Code.
